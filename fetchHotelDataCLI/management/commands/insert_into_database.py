    ...
    def handle(self, *args, **kwargs):
        ...
        if not listings.exists():
            self.stdout.write(self.style.WARNING(
                'No listings found in the scrapdb database.'))
            return
        ...
        for listing in listings:
            with transaction.atomic():
                ...
                existing_hotel = Hotel.objects.filter(
                    title=listing.title).first()

                if existing_hotel:
                    self.stdout.write(self.style.WARNING(
                        f"Hotel with title '{listing.title}' already exists. Skipping..."))
                    continue
                ...
                if isinstance(listing.images, list):
                    ...
                    self.stdout.write(self.style.WARNING(
                        f'No images found or invalid format for {hotel.title}.'))

    def save_image(self, hotel, image_path):
        scrapy_image_dir = settings.SCRAPY_IMAGE_DIR
        full_image_path = os.path.join(
            scrapy_image_dir, image_path.replace(" ", "_"))
        try:
            ...
            new_filename = hotel_image_file_path(
                image_instance, original_filename)
            ...
            image_record = Image.objects.create(
                hotel=hotel, image=new_filename)
            self.stdout.write(self.style.SUCCESS(
                f'Added Image: {image_record.image.url} for Hotel: {hotel.title}'))

        except Exception as e:
            self.stderr.write(self.style.ERROR(
                f'Failed to copy {full_image_path}: {e}'))
